...
"""
    ...
    def generate_geographic_distribution(self):
        ...
        # Call the shared helper function with all outcomes (theme level)
        return self._generate_geographic_distribution_from_outcomes(self.outcomes_df, "theme")
...
"""
    ...
    def _generate_geographic_distribution_from_outcomes(self, filtered_outcomes, chart_type="subportfolio"):
        """Generic helper function that implements the portfolio geographic logic for any outcomes dataset"""
        if filtered_outcomes.empty:
            return ""
        
        # Use the exact same logic as the original portfolio function
        ...
        # Show regional distribution with Global first
        html += "<ul>\n"
        ...
        # Generate the world map if there are specific countries - use exact portfolio logic
        if geo_counts:
            # Use the full portfolio world map implementation
            html += self._generate_world_map_from_data(geo_counts, global_count, total_outcomes, chart_type, country_to_region)
            
        return html
    
    def _generate_world_map_from_data(self, geo_counts, global_count, total_outcomes, chart_type, country_to_region):
        """Extract exact world map generation from portfolio function"""
        # Generate data for the world map - create a mapping object (exact portfolio logic)
        country_mapping = {
            'Tanzania': 'Tanzania',
            'Kenya': 'Kenya', 
            'Indonesia': 'Indonesia',
        ...
        # Generate global indicator 
        global_indicator_html = ""
        if global_count > 0:
            global_indicator_html = f'''
        <div style="
            position: absolute; left: 50%; top: 180px; width: 30px; height: 30px;
            background-color: rgba(0, 123, 255, 0.8); border: 2px solid #007bff;
        ...
        " title="Global: {global_count} outcomes ({(global_count / total_outcomes * 100):.1f}% of all outcomes)">G</div>'''
        
        # Generate unique canvas ID
        import hashlib
        canvas_id = f"worldMap{chart_type.title()}_{hashlib.md5(str(hash(tuple(sorted(geo_counts.items())))).encode()).hexdigest()[:8]}"
        
        # Adjust width based on chart type - Grant pages use full width
        container_width = "width: 100%" if chart_type == "grant" else "max-width: 1000px"
        canvas_style = 'style="width: 100%; height: auto;"' if chart_type == "grant" else ""
        
        html = f'''
<div style="margin: 30px 0;">
    <div style="{container_width}; margin: 0 auto; position: relative;">
        <canvas id="{canvas_id}" width="1000" height="500" {canvas_style}></canvas>{global_indicator_html}
    </div>
</div>

<script>
(function() {{
    let chartJsLoaded = false, chartGeoLoaded = false;
    
    function createMap() {{
        if (!chartJsLoaded || !chartGeoLoaded) return;
        ...
        fetch('https://unpkg.com/world-atlas/countries-50m.json')
            ...
            .then(topology => {{
                ...
                const countryData = new Map([{map_data_js}]);
                ...
                new Chart(ctx, {{
                    ...
                    data: {{
                        ...
                        datasets: [{{
                            ...
                            outline: countries, showOutline: true,
                            backgroundColor: (context) => {{
                                if (!context.dataIndex || context.dataIndex < 0) return 'rgba(211, 211, 211, 0.2)';
                                const feature = context.chart.data.datasets[0].outline[context.dataIndex];
                                if (!feature?.properties) return 'rgba(211, 211, 211, 0.2)';
                                const count = countryData.get(feature.properties.name) || 0;
                                if (count === 0) return 'rgba(211, 211, 211, 0.2)';
                                const maxCount = Math.max(...Array.from(countryData.values()));
                                const opacity = 0.2 + Math.min(count / maxCount, 1) * 0.8;
                                return `rgba(44, 90, 160, ${{opacity}})`;
                            }},
                            borderColor: 'rgba(0, 0, 0, 0.3)', borderWidth: 1,
                            data: countries.map(c => ({{ feature: c, value: countryData.get(c.properties.name) || 0 }}))
                        }}]
                    }},
                    options: {{
                        responsive: false, maintainAspectRatio: false, showOutline: true, showGraticule: false,
                        plugins: {{
                            legend: {{ display: false }},
                            tooltip: {{
                                callbacks: {{
                                    title: ctx => ctx[0].parsed.feature.properties.name,
                                    label: ctx => {{
                                        const count = ctx.parsed.value;
                                        if (count === 0) return 'No outcomes reported';
                                        const pct = ((count / {total_outcomes}) * 100).toFixed(1);
                                        return `${{count}} outcomes (${{pct}}% of {chart_type} outcomes)`;
                                    }}
                                }}
                            }}
                        }},
    ...
    if (typeof Chart !== 'undefined') {{ chartJsLoaded = true; }} else {{
        const script = document.createElement('script');
        script.src = 'https://cdn.jsdelivr.net/npm/chart.js@4.4.0/dist/chart.umd.js';
        script.onload = () => {{ chartJsLoaded = true; createMap(); }};
        document.head.appendChild(script);
    }}
    
    if (typeof ChartGeo !== 'undefined') {{ chartGeoLoaded = true; }} else {{
        const script = document.createElement('script');
        script.src = 'https://cdn.jsdelivr.net/npm/chartjs-chart-geo@4';
        script.onload = () => {{ chartGeoLoaded = true; createMap(); }};
        document.head.appendChild(script);
    }}
    
    if (chartJsLoaded && chartGeoLoaded) createMap();
}})();
</script>'''
    ...
    def generate_subportfolio_geographic_distribution(self, subportfolio_name):
        ...
        return self._generate_geographic_distribution_from_outcomes(filtered_outcomes, "subportfolio")
...
"""
    ...
    def generate_grant_geographic_distribution(self, grant_outcomes):
        """Generate geographic distribution analysis for individual grant page - uses same function as portfolio"""
        if grant_outcomes.empty:
            return "<p>No outcomes available for geographic analysis.</p>\n"
        
        # Call the exact same helper function used by portfolio and theme pages
        return self._generate_geographic_distribution_from_outcomes(grant_outcomes, "grant")
