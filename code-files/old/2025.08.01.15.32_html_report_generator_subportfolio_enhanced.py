    ...
    def generate_grant_outcome_records(self, grant_outcomes):
        ...
        for report_key in sorted_reports:
            ...
            for _, outcome in enumerate(outcomes):
                description = outcome.get('Description of the Reported Outcome', 'No description available')
                domain = outcome.get('Domain of Change', 'Unknown Domain')
                ob_names = outcome.get('Outcome Basket names', '')
                ...
                # Create outcome basket display
                ob_display = ""
                if pd.notna(ob_names) and str(ob_names).strip():
                    obs = [ob.strip() for ob in str(ob_names).split(';') if ob.strip()]
                    if obs:
                        ob_display = f" <em>({'; '.join(obs)})</em>"
                
                # Create domain display
                domain_display = ""
                if pd.notna(domain) and str(domain).strip():
                    domain_display = f" <strong>[{str(domain).strip()}]</strong>"
                
                html += f"<li>{description}{domain_display}{ob_display}</li>\n"
