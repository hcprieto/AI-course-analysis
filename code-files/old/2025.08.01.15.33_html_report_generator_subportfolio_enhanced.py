...
"""
    ...
    def generate_unified_domain_analysis(self, outcomes_df, analysis_level="theme"):
        ...
        header_levels = {
            ...
            "grant": {"domain": "h4", "section": "h5"}  # Fixed to match portfolio/theme structure
        }
        ...
        # Define all possible domains to ensure we show all domains, even with 0 outcomes
        all_domains = [
            "D1 - Stronger agency of young people of all backgrounds to shape their digital futures",
            "D2 - Expanded equitable access to services for young people", 
            "D3 - Collective efforts by relevant stakeholders for joint learning, knowledge exchange and strategic alignment",
        ...
        html = ""
        for domain_full_text in all_domains:
            domain_outcomes = outcomes_by_domain.get(domain_full_text, [])
            ...
            # If no outcomes for this domain, close the list and continue to next domain
            if outcome_count == 0:
                html += "</ul>\n\n"
                continue
            ...
            # Add pie chart for OB distribution (for theme, subportfolio, and grant pages)
            if analysis_level in ["theme", "subportfolio", "grant"]:
...
<div style="margin: 30px 0;">
    <div style="width: 100%; margin: 0 auto; position: relative;">
        <canvas id="worldMap" width="1000" height="500" style="width: 100%; height: auto;"></canvas>{global_indicator_html}
    </div>
</div>
...
"""
    ...
    def generate_grant_pages(self):
        ...
        for gams_code, grant_info in consolidated_grants.items():
            ...
            # Add grant summary as first bullet point if available
            grant_summary = grant_row.get("Grant's Summary (from GAMS)")
            if pd.notna(grant_summary):
                content_html += f"<li><strong>Grant Summary:</strong><br>{grant_summary}</li>\n"
            ...
            # Add Sub-Portfolio Memberships as inline bullet point with comma-separated links
            subportfolio_links = []
            for sp, is_member in grant_info['subportfolios'].items():
                if is_member:
                    sp_filename = self.sanitize_filename(sp)
                    subportfolio_links.append(f"<a href=\"../subportfolios/{sp_filename}.html\">{sp}</a>")
            
            if subportfolio_links:
                content_html += f"<li><strong>Sub-Portfolio Memberships:</strong> {', '.join(subportfolio_links)}</li>\n"
            
            # Add performance ratings as inline bullet point with indented sub-bullets
            content_html += "<li><strong>Performance Ratings:</strong>\n<ul style='margin-top: 5px;'>\n"
            ...
            # Add outcomes listing section
            if len(grant_outcomes) > 0:
                content_html += self.generate_grant_outcome_records(grant_outcomes)
...
"""
    ...
    def generate_grant_cross_domain_summary(self, grant_outcomes):
        ...
        # Most used OBs across all domains in this grant
        html += "<ol>\n"
        for ob_name, count in sorted(all_obs_usage.items(), key=lambda x: x[1], reverse=True)[:15]:  # Top 15
            percentage = (count / total_outcomes * 100) if total_outcomes > 0 else 0
            html += f'<li><strong>{ob_name}</strong>: {count} uses (tagged in {percentage:.1f}% of the outcomes)</li>\n'
    ...
    def generate_grant_outcome_records(self, grant_outcomes):
        """Generate all outcome records section for individual grant, organized by report"""
        html = f"<h2>All Outcomes from this Grant</h2>\n"
        
        # Group outcomes by report
        outcomes_by_report = {}
        for _, outcome in grant_outcomes.iterrows():
            report_name = outcome.get('Report name', 'Unknown Report')
            report_date = outcome.get('Report submission date', '')
            
            # Create a key combining report name and date
            if pd.notna(report_date) and str(report_date).strip():
                report_key = f"{report_name} ({report_date})"
            else:
                report_key = str(report_name)
            
            if report_key not in outcomes_by_report:
                outcomes_by_report[report_key] = []
            outcomes_by_report[report_key].append(outcome)
        
        # Sort reports by date (most recent first) if possible
        def sort_report_key(report_key):
            # Try to extract date for sorting
            if '(' in report_key and ')' in report_key:
                date_part = report_key.split('(')[-1].replace(')', '')
                try:
                    # Try to parse various date formats
                    import datetime
                    for fmt in ['%Y-%m-%d', '%d/%m/%Y', '%m/%d/%Y', '%Y']:
                        try:
                            return datetime.datetime.strptime(date_part.strip(), fmt)
                        except:
                            continue
                except:
                    pass
            return datetime.datetime.min  # Default for unparseable dates
        
        try:
            sorted_reports = sorted(outcomes_by_report.keys(), key=sort_report_key, reverse=True)
        except:
            # Fallback to alphabetical sorting if date parsing fails
            sorted_reports = sorted(outcomes_by_report.keys())
        
        for report_key in sorted_reports:
            outcomes = outcomes_by_report[report_key]
            outcome_count = len(outcomes)
            
            html += f"<h3>{report_key} ({outcome_count} outcome{'s' if outcome_count != 1 else ''})</h3>\n"
            # Process each outcome within this report (matching Portfolio page format)
            for outcome in outcomes:
                # Get full description for the title
                description = outcome.get('Description of the Reported Outcome', 'N/A')
                ...
                # 4. Significance level (if available)
                significance = outcome.get('Significance Level', 'N/A')
                if pd.notna(significance):
                    html += f"<li><strong>Significance level:</strong> {significance}</li>\n"
                ...
                for col in special_cases_cols:
                    ...
                    if pd.notna(value) and str(value).strip() not in ['0', '']:
                        special_case_name = col.replace('Special case - ', '')
                        special_cases.append(special_case_name)
                
                if special_cases:
                    html += f"<li><strong>Special case tags:</strong> {', '.join(special_cases)}</li>\n"
                
                # 6. Quality of Evidence Level (if available)
                quality = outcome.get('Quality of Evidence Level')
                if pd.notna(quality) and str(quality).strip():
                    html += f"<li><strong>Quality of Evidence:</strong> {quality}</li>\n"
                
                # 7. Country(ies) / Global
                countries = outcome.get('Country(ies) / Global')
                if pd.notna(countries) and str(countries).strip():
                    html += f"<li><strong>Countries:</strong> {countries}</li>\n"
                
                # 8. Links to supporting evidence (if available)
                links = outcome.get('Links to supporting evidence')
                if pd.notna(links) and str(links).strip():
                    html += f"<li><strong>Supporting evidence:</strong> {links}</li>\n"
                
                # 9. Notes/insights from the miner (if available)
                notes = outcome.get('Notes/insights from the miner')
                if pd.notna(notes) and str(notes).strip():
                    html += f"<li><strong>Miner notes:</strong> {notes}</li>\n"
