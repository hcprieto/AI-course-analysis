    ...
    def generate_significance_stacked_chart(self, significance_by_domain, domain_totals, overall_significance, total_outcomes):
        ...
        for i, domain_code in enumerate(domain_order):
            if domain_code in significance_by_domain:
                ...
                datasets.append({
                    'label': f'{domain_code}: {domain_name[:50]}...' if len(domain_name) > 50 else f'{domain_code}: {domain_name}',
                    'data': dataset_data,
                ...
                tooltip: {{
                    callbacks: {{
                        afterLabel: function(context) {{
                            ...
                            const domainTotal = {domain_totals}[domainCode] || 1;
...
"""
    ...
    def generate_significance_distribution(self):
        ...
        for _, outcome in self.outcomes_df.iterrows():
            domain = outcome.get('Domain of Change', 'Unknown')
            ...
            if domain not in significance_by_domain:
                significance_by_domain[domain] = {}
                domain_totals[domain] = 0
            
            # Initialize significance level if not seen before
            if significance not in significance_by_domain[domain]:
                significance_by_domain[domain][significance] = 0
            
            # Count this outcome
            significance_by_domain[domain][significance] += 1
