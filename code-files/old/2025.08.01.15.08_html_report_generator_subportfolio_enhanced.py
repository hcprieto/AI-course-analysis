...
"""
        ...
        """Generate geographic distribution analysis for individual grant page - reuses exact portfolio logic"""
        # Use exact same logic as portfolio but with grant_outcomes instead of filtered_outcomes
        filtered_outcomes = grant_outcomes
            """Generate geographic distribution analysis for subportfolio with regional aggregation and world map"""
            # Use helper method to get filtered outcomes for this subportfolio
            filtered_outcomes = self.get_subportfolio_outcomes(subportfolio_name)
...
"""
        ...
        def generate_grant_geographic_distribution(self, grant_outcomes):
            """Generate geographic distribution analysis for individual grant page - reuses portfolio logic"""
            if grant_outcomes.empty:
                return "<p>No outcomes available for geographic analysis.</p>\n"
            
            # Call the same core logic used by portfolio pages by temporarily setting up the environment
            # Save the current outcomes data and replace it with grant outcomes
            original_outcomes = self.outcomes_df
            self.outcomes_df = grant_outcomes
            
            # Create a temporary "fake" subportfolio that contains all grant outcomes
            temp_subportfolio = "temp_grant_portfolio"
            
            # Call the existing portfolio function logic directly
            result = self._generate_geographic_for_outcomes(grant_outcomes, "grant")
            
            # Restore original outcomes data
            self.outcomes_df = original_outcomes
            
            return result

        def _generate_geographic_for_outcomes(self, filtered_outcomes, chart_type):
            """Shared geographic generation logic used by both portfolio and grant pages"""
