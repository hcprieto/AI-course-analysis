...
"""
    ...
    def generate_geographic_distribution_for_outcomes(self, filtered_outcomes, chart_type="subportfolio"):
        """Generic helper function to generate geographic distribution for any set of outcomes"""
        if filtered_outcomes.empty:
            return ""
        ...
        # Generate the world map if there are specific countries
        if geo_counts:
            # Copy the exact portfolio implementation (continuing with the rest of the portfolio function logic)
            # This ensures identical functionality including the global indicator circle
            return self._generate_world_map_html(geo_counts, global_count, total_outcomes, chart_type, country_to_region, html)
        
        return html
    
    def _generate_world_map_html(self, geo_counts, global_count, total_outcomes, chart_type, country_to_region, html):
        """Generate the world map HTML - extracted from portfolio function to ensure identical behavior"""
        
        # Country mapping for chart (from working portfolio function)
        country_mapping = {
            'Tanzania': 'Tanzania', 'Kenya': 'Kenya', 'Indonesia': 'Indonesia', 'Ecuador': 'Ecuador',
            'India': 'India', 'Ghana': 'Ghana', 'Senegal': 'Senegal', 'Mexico': 'Mexico',
            'Philippines': 'Philippines', 'Vietnam': 'Vietnam', 'South Africa': 'South Africa',
            'Rwanda': 'Rwanda', 'Argentina': 'Argentina', 'Nigeria': 'Nigeria', 'Cameroon': 'Cameroon',
            'Sri Lanka': 'Sri Lanka', 'Moldova': 'Moldova', 'Maldives': 'Maldives',
        ...
        # Generate chart with exact portfolio format
        import hashlib
...
(function() {{
    ...
    if (typeof Chart !== 'undefined') {{ chartJsLoaded = true; }} else {{
        loadScript('https://cdn.jsdelivr.net/npm/chart.js@4.4.0/dist/chart.umd.js', () => {{ chartJsLoaded = true; createMap(); }});
    }}
    
    if (typeof ChartGeo !== 'undefined') {{ chartGeoLoaded = true; }} else {{
        loadScript('https://cdn.jsdelivr.net/npm/chartjs-chart-geo@4', () => {{ chartGeoLoaded = true; createMap(); }});
    }}
    ...
    def generate_subportfolio_geographic_distribution(self, subportfolio_name):
        ...
        return self.generate_geographic_distribution_for_outcomes(filtered_outcomes, "subportfolio")
...
"""
    ...
    def generate_grant_geographic_distribution(self, grant_outcomes):
        """Generate geographic distribution analysis for individual grant page - calls same function as portfolio"""
        if grant_outcomes.empty:
            return "<p>No outcomes available for geographic analysis.</p>\n"
        
        # Call the exact same function used by portfolio pages, ensuring identical functionality
        return self.generate_geographic_distribution_for_outcomes(grant_outcomes, "grant")
