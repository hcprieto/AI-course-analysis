            ...
            scales: {{
                ...
                y: {{
                    ...
                    afterFit: function(scale) {{
                        // Calculate maximum stack total to determine appropriate max
                        let maxTotal = 0;
                        ...
                        if (chartData && chartData.labels) {{
                            chartData.labels.forEach((label, index) => {{
                                ...
                                maxTotal = Math.max(maxTotal, stackTotal);
                            }});
                        }}
                        
                        // Add padding based on the scale of the data
                        // For smaller values (< 50), add at least 25% padding
                        // For larger values, 15% padding should be sufficient
                        let paddingPercentage = maxTotal < 50 ? 0.25 : 0.15;
                        let minPadding = maxTotal < 20 ? 5 : (maxTotal < 100 ? 10 : 20);
                        
                        const suggestedMax = Math.max(maxTotal * (1 + paddingPercentage), maxTotal + minPadding);
                        
                        // Let Chart.js determine the appropriate tick interval naturally
...
(function() {{
                    ...
                    data: {{
                        ...
                        datasets: [{{
                            ...
                            backgroundColor: (context) => {{
                                ...
                                const red = Math.floor(40 + intensity * 200); // Range from light to dark red
                                return `rgba(${{red}}, 50, 50, 0.8)`;
                            }},
    ...
    if (typeof Chart !== 'undefined') {{
        chartJsLoaded = true;
        console.log('Chart.js already loaded for subportfolio');
    }} else {{
        loadScript('https://cdn.jsdelivr.net/npm/chart.js@4.2.1/dist/chart.umd.js', () => {{
            chartJsLoaded = true;
            console.log('Chart.js loaded for subportfolio');
    ...
    if (typeof ChartGeo !== 'undefined') {{
        chartGeoLoaded = true;
        console.log('ChartGeo already loaded for subportfolio');
    }} else {{
        loadScript('https://cdn.jsdelivr.net/npm/chartjs-chart-geo@4.2.4/build/chartjs-chart-geo.umd.min.js', () => {{
            chartGeoLoaded = true;
            console.log('ChartGeo loaded for subportfolio');
            createIsolatedSubportfolioMap();
        }});
    }}
