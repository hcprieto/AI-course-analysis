...
"""
    ...
    def generate_unified_domain_analysis(self, outcomes_df, analysis_level="theme"):
        ...
        # Generate analysis for each domain - sort by domain number (D1, D2, D3, D4)
        def domain_sort_key(domain_text):
            """Extract domain number for proper D1, D2, D3, D4 sorting"""
            if ' - ' in domain_text:
                domain_key = domain_text.split(' - ')[0]
                if domain_key.startswith('D') and domain_key[1:].isdigit():
                    return int(domain_key[1:])  # Extract number after 'D'
            return 999  # Put invalid domains at end
        
        html = ""
        for domain_full_text in sorted(outcomes_by_domain.keys(), key=domain_sort_key):
