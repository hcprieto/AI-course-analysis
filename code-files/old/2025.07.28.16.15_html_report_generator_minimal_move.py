...
<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
<script src="https://cdn.jsdelivr.net/npm/chartjs-chart-geo@4.2.4/build/index.umd.min.js"></script>
<script>
...
// Wait for all scripts to load before creating the map
window.addEventListener('load', () => {{
    ...
    // Register ChartGeo components with proper ID handling
    try {{
        // Manually set IDs for controllers that don't have them
        if (ChartGeo.ChoroplethController) {{
            ChartGeo.ChoroplethController.id = 'choropleth';
            Chart.register(ChartGeo.ChoroplethController);
        }}
        
        if (ChartGeo.BubbleMapController) {{
            ChartGeo.BubbleMapController.id = 'bubbleMap';
            Chart.register(ChartGeo.BubbleMapController);
        }}
        
        // Register scales
        if (ChartGeo.ProjectionScale) {{
            ChartGeo.ProjectionScale.id = 'projection';
            Chart.register(ChartGeo.ProjectionScale);
        }}
        
        if (ChartGeo.ColorScale) {{
            ChartGeo.ColorScale.id = 'color';
            Chart.register(ChartGeo.ColorScale);
        }}
        
        if (ChartGeo.SizeScale) {{
            ChartGeo.SizeScale.id = 'size';
            Chart.register(ChartGeo.SizeScale);
        }}
        
        // Register elements
        if (ChartGeo.GeoFeature) {{
            ChartGeo.GeoFeature.id = 'geoFeature';
            Chart.register(ChartGeo.GeoFeature);
        }}
        
        console.log('ChartGeo components registered successfully');
        
        // Verify registration by checking Chart.js registry
        console.log('Available controllers:', Object.keys(Chart.registry.controllers));
        console.log('Available scales:', Object.keys(Chart.registry.scales));
        
    }} catch (e) {{
        console.error('Error registering ChartGeo components:', e);
        return;
    }}
    
    // Add a small delay to ensure registration is complete
    setTimeout(() => {{
        console.log('About to create world map...');
        console.log('Chart registry controllers:', Object.keys(Chart.registry.controllers));
        createWorldMap();
    }}, 100);
    
    // Add interactive behavior to Global indicator
