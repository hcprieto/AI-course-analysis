...
"""
    ...
    def generate_domain_analysis_from_csv(self):
        ...
        for domain_full_text in sorted(outcomes_by_domain.keys()):
            ...
            html += f"<h4>Outcomes assigned to the Domain of Change {domain_key.replace('D', '')}</h4>\n"
            html += f"<h5><strong>{domain_full_text}</strong></h5>\n"
...
"""
    ...
    def generate_significance_distribution(self):
        """Generate theme-wide significance patterns analysis"""
        if self.outcomes_df.empty:
            return ""
        
        # Count significance levels (including null/empty values)
        significance_counts = self.outcomes_df['Significance Level'].value_counts(dropna=False)
        total_outcomes = len(self.outcomes_df)
        
        # Replace NaN with 'Not specified' for better readability
        ...
        html += "To see examples of high significance outcomes, please refer to the pages for the sub-portfolios and grants."
        ...
        for level in significance_order:
            if level in significance_dict:
                count = significance_dict[level]
                percentage = (count / total_outcomes * 100) if total_outcomes > 0 else 0
                html += f"<li><strong>{level}:</strong> {count} ({percentage:.1f}%)</li>\n"
    ...
    def generate_theme_index(self):
        ...
        for sp in self.subportfolios:
            filename = sp.lower().replace(' ', '_').replace('&', 'and')
            content_html += f"""
...
"""
            # Insert the Venn diagram after the Overview heading but before the matrix table
            overview_index = content_html.find("<h2>Overview of Grants and Sub-Portfolios</h2>")
            if overview_index != -1:
                overview_end = content_html.find("</h2>", overview_index) + 5  # After the heading
                content_html = content_html[:overview_end] + venn_html + "<p>Click on grant names to view detailed analysis, or sub-portfolio headers to view portfolio summaries.</p>" + content_html[overview_end:]
...
"""
    ...
    def generate_subportfolio_outcome_records(self, sp_grants, subportfolio_outcomes):
        ...
        for gams_code, grant_info in sp_grants.items():
            ...
            grant_display_name = grant_info['short_name'].replace('_', ' ')
    ...
    def generate_grant_pages(self):
        ...
        for gams_code, grant_info in consolidated_grants.items():
            ...
            # Generate content
            title = grant_info["short_name"].replace('_', ' ')
