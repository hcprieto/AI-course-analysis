...
"""
    ...
    def generate_geographic_distribution(self):
        ...
        # First show global count if any
        if global_count > 0:
            percentage = (global_count / total_outcomes * 100)
            html += f"\n<h3>Global Initiatives</h3>\n"
            html += f"<p><strong>Global:</strong> {global_count} ({percentage:.1f}% of all outcomes)</p>\n"
        ...
        # Create country data for the map
        map_data_js = ',\n                    '.join([
            f'{{feature: "{country_mapping.get(country, country)}", value: {count}}}' 
        ...
        data: {{
            ...
            datasets: [{{
                ...
                backgroundColor: (context) => {{
                    const value = context.parsed;
                    if (!value) return 'rgba(211, 211, 211, 0.3)'; // Light gray for no data
                    const intensity = Math.min(value / 20, 1); // Scale intensity (max at 20 outcomes)
                    return `rgba(44, 90, 160, ${{0.3 + intensity * 0.7}})`; // Blue with varying opacity
                }},
                borderColor: '#2c5aa0',
                borderWidth: 0.5,
                data: [
...
"""
        ...
        html += "\n<h3>Country-Level Distribution</h3>\n<ul>\n"
    ...
    def generate_theme_index(self):
        ...
        # Add custom OBs analysis before geographic distribution
