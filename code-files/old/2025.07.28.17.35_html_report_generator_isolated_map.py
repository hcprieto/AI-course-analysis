...
"""
    ...
    def generate_significance_stacked_chart(self, significance_by_domain, domain_totals, overall_significance, total_outcomes):
        ...
        # Get dynamic domain order from actually discovered domains
        domain_order = sorted(significance_by_domain.keys()) if significance_by_domain else []
...
(function() {{
    ...
    function createIsolatedMap() {{
        ...
        // CRITICAL: Register components IMMEDIATELY and keep them registered
        function ensureChartGeoRegistration() {{
            console.log('=== REGISTRATION DEBUG ===');
            console.log('Chart instance:', Chart);
            console.log('Chart.registry:', Chart.registry);
            console.log('Chart.registry.controllers object:', Chart.registry.controllers);
            console.log('Current controllers:', Object.keys(Chart.registry.controllers || {{}}));
            
            if (typeof ChartGeo !== 'undefined' && ChartGeo.ChoroplethController) {{
                try {{
                    console.log('Attempting registration with controller:', ChartGeo.ChoroplethController);
                    
                    // Force registration every time we need it
                    Chart.register(ChartGeo.ChoroplethController, ChartGeo.GeoFeature, ChartGeo.ColorScale);
                    
                    // CRITICAL FIX: Register in the internal items structure
                    if (Chart.registry.controllers.items) {{
                        Chart.registry.controllers.items.choropleth = ChartGeo.ChoroplethController;
                        console.log('Registered in controllers.items structure');
                    }}
                    
                    if (Chart.registry.elements.items) {{
                        Chart.registry.elements.items.geoFeature = ChartGeo.GeoFeature;
                        console.log('Registered in elements.items structure');
                    }}
                    
                    if (Chart.registry.scales.items) {{
                        Chart.registry.scales.items.color = ChartGeo.ColorScale;
                        console.log('Registered in scales.items structure');
                    }}
                    
                    // Also ensure direct registry assignment as backup
                    Chart.registry.controllers.choropleth = ChartGeo.ChoroplethController;
                    Chart.registry.elements.geoFeature = ChartGeo.GeoFeature;
                    Chart.registry.scales.color = ChartGeo.ColorScale;
                    
                    console.log('Registry controllers.items:', Chart.registry.controllers.items);
                    console.log('Registry elements.items:', Chart.registry.elements.items);
                    console.log('Registry scales.items:', Chart.registry.scales.items);
                    
                    console.log('After registration - controllers:', Object.keys(Chart.registry.controllers || {{}}));
                    console.log('Choropleth controller directly accessible:', Chart.registry.controllers.choropleth);
                    
                    // Test if we can retrieve it the same way Chart.js will
                    try {{
                        const testController = Chart.registry.getController('choropleth');
                        console.log('Retrieved controller via getController:', testController);
                    }} catch (testError) {{
                        console.error('Failed to retrieve via getController:', testError);
                    }}
                    
                    console.log('ChartGeo components force-registered and locked');
                    return true;
                }} catch (regError) {{
        ...
        fetch('https://unpkg.com/world-atlas/countries-50m.json')
            ...
            .then(topology => {{
                ...
                // CRITICAL: Re-register components just before chart creation
                console.log('Re-registering components just before chart creation...');
                ensureChartGeoRegistration();
                
                // Final check before creating chart
                console.log('=== CHART CREATION DEBUG ===');
                console.log('Chart instance about to be used:', Chart);
                console.log('Chart.registry about to be used:', Chart.registry);
                console.log('Controllers available:', Object.keys(Chart.registry.controllers || {{}}));
                console.log('Choropleth controller accessible:', Chart.registry.controllers?.choropleth);
                console.log('Registry getController method:', Chart.registry.getController);
                
                // Try one more direct test
                try {{
                    const directTest = Chart.registry.getController('choropleth');
                    console.log('Direct test SUCCESS - got controller:', directTest);
                }} catch (directError) {{
                    console.error('Direct test FAILED:', directError);
                    console.log('Registry contents at time of failure:', Chart.registry);
                }}
                ...
                const chart = new Chart(ctx, {{
                    ...
                    data: {{
                        ...
                        datasets: [{{
                            ...
                            outline: countries,
                            showOutline: true,
                            // Add required properties for choropleth
                            datasetElementType: 'geoFeature',
                            dataElementType: 'geoFeature',
...
"""
    ...
    def generate_theme_index(self):
        ...
        # Add geographic distribution as h3 subsection within outcomes (moved from end)
        content_html += self.generate_geographic_distribution()
        
        # Add Outcome Basket analysis main section (moved to after geographic distribution)
