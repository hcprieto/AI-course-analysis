...
"""
    ...
    def generate_special_cases_analysis(self):
        ...
        # Generate stacked chart showing distribution across domains
        # Prepare data for chart
        domains = sorted(special_cases_by_domain.keys())
        special_case_types = sorted(special_cases_totals.keys())
        
        # Create datasets for stacked chart
        chart_datasets = []
        colors = ['#dc3545', '#fd7e14', '#ffc107', '#28a745', '#17a2b8', '#6f42c1']
        
        for i, case_type in enumerate(special_case_types):
            dataset_data = []
            for domain in domains:
                count = special_cases_by_domain.get(domain, {}).get(case_type, 0)
                dataset_data.append(count)
    ...
    <div style="max-width: 900px; margin: 0 auto;">
        <canvas id="specialCasesStackedChart" width="800" height="500"></canvas>
    </div>
</div>

<script>
    const specialCasesCtx = document.getElementById('specialCasesStackedChart').getContext('2d');
    const specialCasesChart = new Chart(specialCasesCtx, {{
        type: 'bar',
        data: {{
            labels: {domains},
        ...
        options: {{
            ...
            plugins: {{
                title: {{
                    display: true,
                    text: 'Special Cases Distribution Across Domains of Change',
                    font: {{
                        size: 16,
                        weight: 'bold'
                    }}
                ...
                tooltip: {{
                    callbacks: {{
                        label: function(context) {{
                            const domainCode = context.label;
                            const domainTotal = {dict(domain_totals)}[domainCode] || 1;
                            const value = context.parsed.y;
                            const percentage = ((value / domainTotal) * 100).toFixed(1);
                            return context.dataset.label + ': ' + value + ' (' + percentage + '% of ' + domainCode + ' outcomes)';
                        }}
                    }}
                }}
            }}
